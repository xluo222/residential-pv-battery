...
\begin{abstract}
will write abstract after results + model get verified with mentor. 
talk about the importance of researching pv-battery systems. how this study focuses on cost minimization model for homeowners and examines summer vs winter months + battery capacities using hourly residential load profiles, solar generation profiles, time-of-use pricing, and export credits in model. very briefly summarize results and conclude with something about the significance of these results. recommendations for homeowners on optimizing their batteries. just an outline of abstract and testing out the spacing for the rest of my sections and graphs by using placeholder text here

don't forget to do references in bibtex
\end{abstract}
...
This study combines publicly available datasets describing residential electricity demand, PV generation, time-of-use tariffs, and battery export compensation. Representative residential load profiles were obtained from the Open Energy Data Initiative's residential hourly load profiles, while hourly PV generation was generated using the National Renewable Energy Laboratory (NREL) PVWatts model. Time-of-use electricity tariffs and Net Billing Tariff (NBT) export prices for San Diego Gas \& Electric (SDG\&E) were used to represent the economic environment for residential PV customers. These four datasets and pricing policies were integrated in the cost-minimization model to examine which battery dispatch strategy would minimize total costs on weekdays for homeowners. Our study focuses on weekdays because time-of-use electricity pricing in San Diego are generally higher on weekdays than on weekends. Modeling weekday electricity consumption therefore captures periods when battery storage has the greatest potential to reduce electricity costs through load shifting.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.5\textwidth]{tou_pricing.png}
...
C_{\mathrm{net}}
=
\left(C_{\mathrm{battery}} + C_{\mathrm{installation}}\right)
(1-I),
\end{equation}
...
\section{Results}
Table \ref{tab:optimal_results} summarizes the results for the optimal battery in each season, including the optimal battery capacity, the daily operating cost after optimization, daily bill savings relative to the no-battery case, annualized battery capital cost, and the resulting daily and annual net benefits.
...
Summarize the model's findings and their significance, how it applies to homeowners and any future studies (more regions, different modeling methods). Limitations -> only case study of one region, so be careful about generalizing results to all regions. 
